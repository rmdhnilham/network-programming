        ...
        while True:
            ...
            if command[-2:]=="xx":
                ...
                if command['perintah']=="upload":
                    ...
                    print(hasil)
                    self.connection.sendall(hasil.encode())
                    filee=open('protocol-storage/upload_'+ command['filename'],'wb')
                ...
                if command['perintah']=="download":
                    hasil = pm.proses(command)
                    if hasil != "null":
                        file_download = command['filename']
                        print("file yang akan di download: "+file_download)
                        #jika data ada
                        if os.path.isfile(file_download):
                            self.connection.sendall(hasil.encode())
                            print("file akan didownload")
                            if hasil!="null":
                                with open(file_download, 'rb') as file_to_send:
